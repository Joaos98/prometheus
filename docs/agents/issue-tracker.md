...
- `docs/specs/0003-v1-2-one-off-marks-and-income-totals/spec.md` — v1.2, the roadmap's v1.2 block refined. `done`, shipped as v1.2. Two items, not the three the roadmap listed: opening an empty Month was rejected during refinement and `docs/roadmap.md` carries the reasoning.
- `docs/specs/0003-v1-2-one-off-marks-and-income-totals/issues/01…04` — four tickets, each item split at the domain/UI boundary: `01` and `03` are the engine halves, `02` and `04` their dashboard halves. All `done`; each one's comments record what was built and how it was checked.
- `docs/specs/0004-v2-composites-categories-and-trends/spec.md` — V2 refined: composite expenses, categories, payment methods and trends, as one spec with four sections. `ready-for-agent`. Supersedes three of spec 0001's *Out of Scope* entries and, in one respect, ADR-0008's dependency count.
- `docs/specs/0004-v2-composites-categories-and-trends/issues/01…14` — fourteen tickets in dependency order. Three chains start in parallel: composites (`01`–`04`), categories and payment methods (`05`–`09`), and the trends view (`10`–`13`). They meet at `12`, which needs the category picker and the trends shell, and at `14`, the demo seed, which needs everything. `01`–`10` are `done`, so the composites chain and the categories-and-payment-methods chain have both landed in full and the trends view has its shell, its shared axis and the one chart component the rest fill; `11`–`14` are still `ready-for-agent`.
