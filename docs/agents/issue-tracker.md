...
- `docs/specs/0004-v2-composites-categories-and-trends/issues/01…14` — fourteen tickets in dependency order. Three chains start in parallel: composites (`01`–`04`), categories and payment methods (`05`–`09`), and the trends view (`10`–`13`). They meet at `12`, which needs the category picker and the trends shell, and at `14`, the demo seed, which needs everything. `01`–`12` are `done`, so the composites chain and the categories-and-payment-methods chain have both landed in full and the trends view has five of its six charts; `13` and `14` are still `ready-for-agent`.
