...
- [x] Opening a Month marks every copied row Unreviewed
- [x] Editing a row clears its Unreviewed mark
- [x] A member can confirm a row that is correct as inherited without editing it, and that clears the mark too
- [x] The Month reports how many rows remain Unreviewed, and the count reaches zero when every row has been edited or confirmed
- [x] The review meter is in the pinned rail
- [x] Each row shows its own review state, in place
- [x] A Pending row — no amount at all — renders as a warning
- [x] An explicitly entered zero is not Pending and does not render as a warning
- [x] Unreviewed and Pending are visibly different states, and a row can be Unreviewed while holding a perfectly good number
- [x] Review state is Household data, the same for everyone looking at the Month, and survives a round-trip through the storage port
