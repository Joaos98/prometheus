...
export function useApi(s: State) {
  ...
  return {
    ...
    async submitIncome(name: string, memberId: string, amountCents: number, effectiveFrom: string, restrictedUse: boolean, oneOff: boolean) { const src = (await http.post("/api/income-sources", { memberId, name, amountCents, effectiveFrom, oneOff, restrictedUse })) as IncomeSource; s.incomeSources.value = [...s.incomeSources.value, src]; return src; },
    ...
    async submitExpense(name: string, participants: string[], splitRule: Record<string, unknown>, effectiveFrom: string, oneOff: boolean) { const e = (await http.post("/api/expenses", { name, participants, splitRule, effectiveFrom, oneOff })) as Expense; s.expenses.value = [...s.expenses.value, e]; return e; },
    ...
    async submitGoal(name: string, participants: string[], splitRule: Record<string, unknown>, targetAmountCents: number | undefined, startAmountCents: number | undefined, effectiveFrom: string) { await http.post("/api/goals", { name, participants, splitRule, targetAmountCents, startAmountCents, effectiveFrom }); await loadAll(); },
